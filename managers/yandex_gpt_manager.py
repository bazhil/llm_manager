import os
import logging

from dotenv import load_dotenv
from yandex_cloud_ml_sdk import YCloudML


load_dotenv()
logger = logging.getLogger(__name__)


class YandexGPTManager:
    """Manage the YandexGPT API"""

    _sdk = None

    @classmethod
    def _get_sdk(cls):
        """Lazy initialization of YandexGPT SDK"""
        if cls._sdk is None:
            folder_id = os.getenv("YANDEX_GPT_FOLDER_ID")
            api_key = os.getenv("YANDEX_GPT_API_KEY")
            logger.info("Initializing Yandex GPT SDK...")
            logger.info(f"  Folder ID: {'SET' if folder_id else 'NOT SET'}")
            logger.info(f"  API Key: {'SET' if api_key else 'NOT SET'}")
            if not folder_id or not api_key:
                logger.error("YANDEX_GPT_FOLDER_ID и YANDEX_GPT_API_KEY должны быть установлены")
                raise ValueError("YANDEX_GPT_FOLDER_ID и YANDEX_GPT_API_KEY должны быть установлены")
            cls._sdk = YCloudML(folder_id=folder_id, auth=api_key)
            logger.info("Yandex GPT SDK initialized successfully")
        else:
            logger.debug("Using existing SDK instance")
        return cls._sdk

    @classmethod
    async def get_response(cls, system_prompt: str, prompt: str):
        """Get response from the YandexGPT"""
        logger.info("="*60)
        logger.info("GENERATING RESPONSE - Yandex GPT")
        logger.info("Model: yandexgpt")
        logger.info(f"Prompt length: {len(prompt)} chars")
        logger.info(f"System prompt length: {len(system_prompt)} chars")

        sdk = cls._get_sdk()
        logger.info("Sending request to Yandex GPT API...")
        response = sdk.models.completions("yandexgpt").configure(temperature=0.5).run(
            messages=[
                {"role": "system", "text": system_prompt},
                {"role": "user", "text": prompt}
            ]
        )

        result = ""
        for alternative in response:
            result += alternative.text

        logger.info(f"Response received: {len(result)} chars")
        logger.info("="*60)
        return result
